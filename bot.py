...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    processing_msg = await update.message.reply_text(
        bold_all(f"`⏳ Pʀᴏᴄᴇꜱꜱɪɴɢ​ {len(urls)} ​Lɪɴᴋ​​{'s' if len(urls) > 1 else ''}...`"),
        parse_mode=ParseMode.MARKDOWN_V2,
    )
...
async def cmd_restart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user.id):
        await update.message.reply_text(bold_all(f"🚫 {sc('Admin only command.')}"), parse_mode=ParseMode.MARKDOWN_V2)
        return
    await update.message.reply_text(bold_all(sc("Restarting Engine...⏳")), parse_mode=ParseMode.MARKDOWN_V2)
    ...
    for admin_id in admin_ids:
        try:
            await app.bot.send_message(
                admin_id,
                bold_all(sc("Engine restarted successfully!!! 🚀")),
                parse_mode=ParseMode.MARKDOWN_V2,
            )
        except Exception as e:
